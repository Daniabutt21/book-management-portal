...
export default function AdminFeedbackPage() {
  ...
  const filteredFeedbacks = tab === 0 ? feedbacks : tab === 1 
  ...
  return (
    <DashboardLayout>
      <Box sx={{ p: { xs: 2, sm: 3, md: 4 } }}>
        <Typography variant="h4" fontWeight={700} color="#2d3748" sx={{ mb: 1, fontSize: { xs: '1.5rem', sm: '1.75rem', md: '2rem' } }}>
          Moderate Reviews
        </Typography>
        ...
        <Box sx={{ mb: 4 }}>
          <Tabs value={tab} onChange={(e, val) => setTab(val)}
            ...
            <Tab label={`Pending (${feedbacks.filter(f => !f.isApproved).length})`} />
            <Tab label={`Approved (${feedbacks.filter(f => f.isApproved).length})`} />
          </Tabs>
        </Box>
        ...
        {filteredFeedbacks.length === 0 ? (
          <Card sx={{ textAlign: 'center', py: 6, border: '1px solid #e2e8f0', boxShadow: 'none' }}>
            <Typography variant="h6" color="text.secondary">
              {tab === 1 ? 'No pending reviews' : tab === 2 ? 'No approved reviews' : 'No reviews yet'}
            </Typography>
          </Card>
        ) : (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {filteredFeedbacks.map(feedback => (
              <Card key={feedback.id} sx={{ border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                <Box sx={{ p: { xs: 2.5, sm: 3 } }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2, gap: 2, flexWrap: 'wrap' }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      <Rating value={feedback.rating} readOnly size="small" sx={{ color: '#f59e0b' }} />
                      {feedback.isApproved ? (
                        <Chip label="Approved" size="small" sx={{ backgroundColor: '#10b981', color: 'white', fontWeight: 600 }} />
                      ) : (
                        <Chip label="Pending" size="small" sx={{ backgroundColor: '#f59e0b', color: 'white', fontWeight: 600 }} />
                      )}
                    </Box>
                  </Box>

                  <Box sx={{ mb: 2, p: 2, backgroundColor: '#f7fafc', borderRadius: 2, borderLeft: '3px solid #3B82F6' }}>
                    <Typography variant="caption" fontWeight={600} color="#718096" sx={{ textTransform: 'uppercase', fontSize: '0.7rem', display: 'block', mb: 0.5 }}>
                      Book
                    </Typography>
                  ...
                  <Typography variant="body2" color="#4a5568" sx={{ lineHeight: 1.7, mb: 2, fontSize: { xs: '0.875rem', sm: '0.95rem' } }}>
                    {feedback.comment}
                  </Typography>
